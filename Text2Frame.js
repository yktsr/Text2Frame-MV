 ...
 *   テキストとゲームの同期を開始
 *
 *  基本的にはデフォルト設定での実行を推奨していますが、プラグインパラメータから
 *  機能を制御できます。
 *  このリアルタイム双方向シームレス同期特有のプラグインパラメータは、
 *  「同期の向き」だけです。その他、
 *    - 取り込み元フォルダ名
 *    - 反映方法
 *    - 反映結果をテキストに書き戻す
 *  を指定可能です。ただし、反映方法に限り、 add を選んだ場合はエラーとなります。
 *  「同期の向き」では、以下の3種類の同期の方向を選べます。
 *    - both: 双方向
 *    - push: テキスト→ゲームだけ
 *    - pull: ゲーム→テキストだけ
 *
 *  プラグインコマンドの引数を記述することで、プラグインパラメータで指定された
 *  値を上書きすることができます。
 *
 *  例1: textフォルダ内のテキストを監視し、テキストかイベントが修正されると
 *      統合モードで両者に反映される（同期される）
 *    START_DATA_SYNC both text merge always
 *    テキストとゲームの同期を開始 双方向 text 統合 常に書き戻す
 *
