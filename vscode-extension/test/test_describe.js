...
describe('describeRef', function () {
  ...
  before(function () {
    ...
    fs.writeFileSync(path.join(dir, 'Actors.json'), JSON.stringify([null, { id: 1, name: 'ハロルド', faceName: 'Actor2', faceIndex: 0 }]))
    fs.writeFileSync(path.join(dir, 'MapInfos.json'), JSON.stringify([null, { id: 1, name: 'はじまりの村' }]))
  ...
  it('shows the name next to a named id', function () {
    ...
    expect(info.hint).to.equal('村人と話した')
  ...
  it('lists the other ids that share the name', function () {
    expect(describeRef(db, { kind: 'switch', id: 2 }).lines).to.eql(['橋がかかった', '同じ名前のスイッチ: 0004'])
  })
  ...
  it('describes a range by its first name and count, and flags the missing part', function () {
    ...
    expect(info.hint).to.equal('村人と話した…(3件)')
    expect(info.lines).to.eql(['0001 村人と話した', '0002 橋がかかった', '0003 (名前なし)'])
  ...
  it('does not put a text hint on a face, and checks the sheet and the number', function () {
    const ok = describeRef(db, { kind: 'face', id: 0, faceName: 'Actor2' }, { exists: () => true })
    expect(ok.hint).to.equal(undefined)
    expect(ok.lines).to.eql(['ハロルド の既定の顔'])
    expect(describeRef(db, { kind: 'face', id: 0, faceName: 'nobody' }, { exists: () => false }).problem.severity).to.equal('warning')
    expect(describeRef(db, { kind: 'face', id: 8, faceName: 'Actor2' }, { exists: () => true }).problem.message).to.contain('0〜7')
  })

  it('names an event from the map of the text, and warns about one the map does not have', function () {
    const events = { mapId: 1, events: [null, { name: '宝箱', x: 8, y: 11 }, { name: '', x: 0, y: 3 }, null] }
    const named = describeRef(db, { kind: 'event', id: 1 }, { events })
    expect([named.hint, named.title, named.lines]).to.eql(['宝箱 (8,11)', 'イベント EV001', ['宝箱', '座標 (8,11)', 'マップ 0001 はじまりの村']])
    expect(describeRef(db, { kind: 'event', id: 2 }, { events }).hint).to.equal('(名前なし) (0,3)')
    const missing = describeRef(db, { kind: 'event', id: 3 }, { events })
    expect(missing.problem.message).to.equal('イベント EV003 はマップ 0001 はじまりの村にありません')
