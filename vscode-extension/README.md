...
**RPG ツクール MV / MZ のイベントを、VS Code でテキストとして書き、デバッグするための拡張機能です。**

この拡張では、Text2Frame の記法を使い、ツクールのイベント編集画面で1コマンドずつ入力する代わりに、セリフや選択肢をふつうの文章のように書けます。
書いたものを、ボタンひとつでゲームに入れて、そのまま VS Code の中でテストプレイできます。
...
| 🔍 **調べる** | マップ・イベント・コモンイベントの一覧、データベースの一覧、スイッチや変数を使っている所、マップのつながり、プロジェクト全体の検査 |
...
- いちばん上の `---` で囲まれた部分は **宛先のメモ**（front matter）です。このテキストをゲームのどこに戻すかが書いてあります（`kind: event` はマップのイベント、`kind: common` はコモンイベント）。**書き換えないでください。** 別のイベントに書き込まれてしまいます。
...
| メッセージのはみ出し | 「ここで改行する」 |
| 衝突の目印（[両方で変えていたとき](#両方で変えていたとき衝突)） | 「テキストの方を残す」「ゲームの方を残す」「両方残して、目印だけ消す」 |
...
反映も取り出しも、既定は **統合（merge）** です。片方の内容で相手を置き換えるのではなく、両方の編集を残して合わせます。
テキストを正として全部置き換える **上書き（overwrite）** にするには、反映は設定 `text2frame.strategy` を `overwrite` に、取り出しは「操作」→「上級」→「全部取り直す(上書き)」を使います（ツクールで直したところ・テキストに書いたところは、それぞれ消えます）。

| 対象 | 押す場所 |
| --- | --- |
| 開いているテキスト | エディタの右上、または「操作」→「よく使う」の「ゲームに反映(このファイル)」「ゲームから取り出す(このファイル)」 |
| テキストのフォルダ全部 | 「操作」→「まとめて反映・取り出す」の「すべてゲームに反映」「すべてゲームから取り出す」（トップの README やヘルプでいう一括反映・一括取り出しです） |
| 一覧の中の1ページ | 「Maps / Common Events」一覧のページの行にマウスを合わせると出る「ゲームに反映」「ゲームから取り出す」のボタン |

**「すべてゲームから取り出す」で何を作るか**は、設定 `text2frame.exportScope` で選べます。既定は「中身のあるものだけ」です（イベントが数千あるゲームでは、全部作ると探せなくなるためです）。**見出し情報付きテキストは、どれを選んでも必ず更新されます。**

| 選ぶもの | 作るもの |
| --- | --- |
| 中身のあるものだけ（既定） | コマンドが1つでもあるページ・コモンイベント |
| 会話があるものだけ | 文章・選択肢・スクロールを含むものだけ |
| 見出し情報付きテキストだけ | 新しくは作らない（見出し情報付きテキストだけ更新） |
| 全部 | 空のページも含めて全部 |

新しく作るテキストの名前は `map001-はじまりの村_event004-宝箱_page2.txt` のように、ツクールで付けたマップ名・イベント名が入ります。**作ったあとは自由に名前を変えたり、フォルダに分けたりできます。** 取り出しは、先頭の宛先のメモ（front matter）を見て同じファイルに書き続けます。
...
これができるのは、テキストとゲームが最後にそろっていた状態を **祖先** として、ゲームのフォルダの `.t2f-base` に控えているからです。
反映・取り出しのたびに、祖先・テキスト・ゲームの3つを突き合わせて（3-way マージ）、どちらが何を変えたのかを見分けています。
祖先の置き場所は CLI やツクールのプラグインコマンドと同じなので、混ぜて使っても続きから合わせられます。
...
- 「**履歴**」の欄（または「操作」→「上級」→「編集履歴を表示する」）に、操作が新しい順に並びます。行には始めた時刻と、書き換わったものの内訳（「ゲームのデータ 2件」「テキスト 1件」）が出ます。開くと書き換わったファイルが並び、押すと差分が出ます。
- 操作の行の「**巻き戻す**」（アイコン、または右クリック →「この操作の直前まで巻き戻す」）で、**その操作を始める直前まで戻ります**。**その操作より後の操作も、一緒に取り消されます**（1つの操作だけを選んで取り消すのではありません）。何がいくつ戻るか、後ろのいくつの操作が取り消されるかは、押す前の確認に出ます。
- **ゲームのデータが戻るページは、テキストもその内容に作り直してそろえます。** 反映はテキストを書き換えないので、ゲームのデータだけを戻すと「テキストは新しいまま、ゲームは古いまま」で食い違います。そろえるので、巻き戻したあとはテキストとゲームの中身が一致します。**テキストに残っていた未反映の編集は、ゲームの内容に置き換わります**（書き込む前に差分で確かめられます）。
- テキストだけが戻る行（取り出しの巻き戻し）では、ゲームのデータは動きません。取り出す前はテキストとゲームが違っていたので、その状態に戻します。
- その時点より後に作られたファイルは、消さずに残します。残したものは名前を挙げて知らせます。
- 巻き戻したことも1つの操作として履歴に残るので、その行で「巻き戻す」を押せば元に戻せます（テキストの作り直しも一緒に取り消されます）。
- 控えはゲームのフォルダの `.t2f-history` に置きます。残すのは最新の100回ぶんです（設定 `text2frame.history.keep`）。消してもゲームの動きには関係しません。
- 手で書き換えただけ（まだ反映していない）のテキストは控えていません。その編集を戻すときは、エディタの「元に戻す」や VS Code の「タイムライン」を使ってください。
...
テキストがまだ無いページの行を押すと、**ファイルは作らずに、ゲームのデータから読んだ中身を読み取り専用で開きます**。書き換えたくなったら、その画面の右上の「このイベントをゲームから取り出す」を押してください。そのときに初めてテキストができます。
...
- 右クリックで「イベントの前に立つ」「このページをすぐ実行」「マップのつながりを見る」「マップのつながりを図で見る」を選べます。
...
| 設定 | 既定 | 決めること |
| --- | --- | --- |
| `text2frame.exportScope` | 中身のあるものだけ | 「すべてゲームから取り出す」で何を作るか。見出し情報付きテキストは、どれを選んでも更新されます |
...
| テキストに衝突の目印が入っている | 両方で同じ所を変えた印です。残したい方だけにして衝突の目印を消し、もう一度反映してください（[両方で変えていたとき](#両方で変えていたとき衝突)） |
| 取り出したら、書いた文が消えた | 「上級」の「全部取り直す(上書き)」は、統合（merge）ではなく上書き（overwrite）で、テキストを全部置き換えます。ふだんは「ゲームから取り出す」を使ってください。[履歴](#前の状態に戻す履歴)から戻せます |
