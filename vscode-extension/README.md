...
- 「**履歴**」の欄（または「操作」→「上級」→「履歴(前の状態に戻す)」）に、操作が新しい順に並びます。開くと、書き換わったファイルが並び、押すと差分が出ます。行には「テキスト」「ゲームのデータ」のどちらかが書いてあります。
- 操作の行を右クリック →「**この時点に戻す**」で、**その操作を始める直前の状態に戻ります**。その時点より後に書き換わったものは、テキストもゲームのデータもまとめて戻ります（1つの操作だけを選んで取り消すのではありません）。
- その時点より後に作られたファイルは、消さずに残します。残したものは名前を挙げて知らせます。
- 戻したことも1つの操作として履歴に残るので、その行で「この時点に戻す」を押せば元に戻せます。
