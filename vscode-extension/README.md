...
2. 操作する。入口は3つ:
   ...
   - エディタ右上のボタン（現在ファイルの Deploy / Export など）
...
- **祖先あり → 3-way マージ**: テキスト編集と（ツクール UI 側の）JSON 編集を統合。同じ箇所を双方が別々に変えた時だけ**両方残す**（「テキストの変更 / ゲームの変更」の目印コメント＋警告）。
- **祖先なし → overlay**: JSON 構造を保ち会話文字列だけ差し替え（祖先がまだ無いときはここ）。祖先は **Export All / Export（pull）/ Seed Locale / デプロイ成功**のいずれでも保存されるため、たとえば「Export All → 翻訳 → Deploy」なら**編集後の最初のデプロイから 3-way**になります。
- **対象が空 → 全反映**。
...
- `onConflict`（既定）: 衝突が出た回だけ、テキストにマージ結果を書き戻します。両方残しブロックは次の目印コメントで囲まれます（タグ記号 `< >` は使わないので文法警告は出ません）:
  ```
  === テキストの変更 / from text ===
  （テキスト側の内容）
  === ゲームの変更 / from game ===
  ...
  ```
  ライターはテキスト上でどちらか＋目印3行を消して解決できます。
...
3. **ゲームへ反映（Deploy Locale）** — 「Text2Frame: ロケールをデプロイ / Deploy Locale」を実行します。
   ...
   - **merge**: JSON 構造を保持し会話文字列だけ差し替え。祖先(`.t2f-base/`＝デプロイ/書き出し成功時に自動保存)があれば**ライター編集と UI 編集を統合する 3-way**（同じ箇所を双方が別々に変えた時だけ**両方残す**：「テキストの変更 / ゲームの変更」の目印コメント＋警告）。祖先が無ければ overlay に自動フォールバック。反映成功後は現在のテキストが新しい祖先になります。
