...
**「すべてゲームから取り出す」で何を作るか**は、設定 `text2frame.exportScope` で選べます。既定は「中身のあるものだけ」です（イベントが数千あるゲームでは、全部作ると探せなくなるためです）。**既にあるテキストは、どれを選んでも必ず更新されます。**

| 選ぶもの | 作るもの |
| --- | --- |
| 中身のあるものだけ（既定） | コマンドが1つでもあるページ・コモンイベント |
| 会話があるものだけ | 文章・選択肢・スクロールを含むものだけ |
| テキストがあるものだけ | 新しくは作らない（今あるテキストだけ更新） |
| 全部 | 空のページも含めて全部 |
...
- 「**履歴**」の欄（または「操作」→「上級」→「履歴(前の状態に巻き戻す)」）に、操作が新しい順に並びます。行には始めた時刻と、書き換わったものの内訳（「ゲームのデータ 2件」「テキスト 1件」）が出ます。開くと書き換わったファイルが並び、押すと差分が出ます。
...
| 設定 | 既定 | 決めること |
| --- | --- | --- |
| `text2frame.exportScope` | 中身のあるものだけ | 「すべてゲームから取り出す」で何を作るか。既にあるテキストは、どれを選んでも更新されます |
