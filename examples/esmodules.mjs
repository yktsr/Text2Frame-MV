/*
 * Text2Frame-MV をライブラリとして使う例(ES Modules)。公開している関数をすべて使います。
 *
 *   npm install @yktsr/text2frame-mv
 *   node esmodules.mjs
 *
 * ファイルを読み書きする関数も使うので、パッケージ本体を import します。
 * 同梱の Text2Frame.es.mjs は、ファイルを扱わない関数(compile など)だけを使うときのものです。
 *
...
import T2F from '@yktsr/text2frame-mv' // テキスト -> イベント(反映)
import F2T from '@yktsr/text2frame-mv/Frame2Text.js' // イベント -> テキスト(取り出し)
